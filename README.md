...
- **읽기 전용 트랜잭션** — LLM이 생성한 조회 쿼리를 PostgreSQL 읽기 전용 트랜잭션으로 실행해, 정적 검증과 별개로 INSERT·UPDATE·DELETE 등 모든 쓰기를 DB 엔진 차원에서 차단합니다. 애플리케이션 검증(7단계 정적 검증)과 DB 차원 방어를 결합한 이중 안전장치입니다.
...
- 도메인 데이터를 추가 확보해 SQL 생성 프롬프트와 비즈니스 로직 규칙을 정밀하게 개선하고, 누적된 피드백·RAG score 데이터를 기반으로 refine·rerank 임계값을 자동으로 조정하는 파이프라인을 구축합니다.
