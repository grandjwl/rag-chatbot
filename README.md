...
### 프로젝트 개요
![image](<img width="601" height="302" alt="image" src="https://github.com/user-attachments/assets/b484a8ad-a78c-4274-a99c-12621a26d8c1" />)
...
- 전체 시스템은 사용자 요청을 받는 **React 프론트엔드**, API 게이트웨이 역할의 **FastAPI 백엔드**, 자연어 처리 핵심 엔진인 **LLM 서버**, 그리고 **PostgreSQL + ChromaDB** 데이터 계층으로 구성됩니다.
- 각 컴포넌트는 Docker로 컨테이너화되어 있으며, Cloudflare 터널을 통해 외부에서 내부 DB로 안전하게 접근합니다.
- 본 레포지터리는 이 중 **LLM 서버** 부분의 설계 및 구현을 다룹니다.
<br><br>

### 프로젝트 환경
![image](여기에_PPT_9페이지_기술스택_이미지)

- LLM 서버는 **Python**과 **FastAPI**로 구축했으며, 에이전트 파이프라인은 **LangGraph**로 노드 간 흐름과 분기 조건을 관리합니다.
- 자연어 처리에는 **Gemini 2.5 Flash**(LLM 및 임베딩)와 **Cohere Reranker**(검색 재정렬)를 활용했습니다.
- 데이터베이스는 정형 데이터용 **PostgreSQL**과 벡터 검색용 **ChromaDB**를 함께 사용하여 RAG 파이프라인을 구성했습니다.
<br><br>
